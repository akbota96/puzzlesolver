...
I represented it as a 2D array.
...
It consists of the parent node of the current node, the board of the current state node
and the number of moves up to the current state node.
...
To identify whether a puzzle is unsolvable, we need to look at two things: oddness of the board size
and number of inversions.

If a board is odd and number of inversions is odd -> puzzle is unsolvable.
If a board is even and (number of inversions + number of the row where value 0 resides) is even -> puzzle is unsolvable.
...
I would prefer to have a better priority function due to the fact that for each
board value we would need to go through n^2 iterations, which is a bit time-consuming.
...
More complex puzzles would require bigger RAM size.
